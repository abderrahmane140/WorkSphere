    ...
    const randerUnassignedStaff = () => {
        ...
        unsignedworkerOnly.forEach(Worker => {
            ...
            div.dataset.workedId = Worker.id
...
const renderWorkersInRooms = () => {
    rooms.forEach(room => {
        ...
        if (!container.dataset.dragInit) {
            // dragover: allow drop & highlight parent zone
            container.addEventListener('dragover', (e) => {
                e.preventDefault();
                // highlight the parent zone (not container) for clearer UI
                if (container.parentElement) container.parentElement.classList.add('bg-blue-100/10');
            });

            // dragleave: remove highlight from parent
            container.addEventListener('dragleave', () => {
                if (container.parentElement) container.parentElement.classList.remove('bg-blue-100/10');
            });

            // drop: handle dropped worker id
            container.addEventListener('drop', (e) => {
                e.preventDefault();
                if (container.parentElement) container.parentElement.classList.remove('bg-blue-100/10');
                const text = e.dataTransfer.getData('text/plain');
                const workerId = Number(text);
                // call addWorkerToRoom with numeric id and room id
                addWorkerToRoom(workerId, room.id);
            });

            container.dataset.dragInit = '1';
        }

        // render current assigned workers for this room
        const assignedWorkers = workers.filter(w => w.assignedZone === room.id);

        assignedWorkers.forEach(worker => {
            const div = document.createElement('div');
         div.className = 'worker-card relative inline-block m-1 cursor-move';
        div.setAttribute('draggable', 'true');
        ...
        div.innerHTML = `
            <div class="relative">
                ...
                <img src="${worker.image}" 
                    alt="${worker.name}" 
                    class="w-12 h-12 rounded-full object-cover border-3 border-gray-200 shadow">

                <!-- Remove button (top-right overlay) -->
                <button type="button"
                    class="remove-worker-btn absolute -top-1 -right-1 w-5 h-5 flex items-center justify-center 
                        bg-red-600 text-white rounded-full text-[15px] shadow hover:bg-red-700 transition cursor-pointer    ">
                    &times;
                </button>
            </div>
        `;
            ...
            div.querySelector('.remove-worker-btn').addEventListener('click', (e) => {
                e.stopPropagation();
                removeWorkerFrZoon(worker.id);
            });
